...
\begin{frame}{The idea behind AdaBoost}
%
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{figure}
...
\begin{tikzpicture}
	...
	%	\node[rectangle,fill=pink!80] (G) at (1.5,-6) {$G(x)=\sign\Bigl(\sum_{m=1}^M\beta_mG_m(x)\Bigr)$};
	\node[rectangle,fill=mLightGreen!20,font=\large] (G) at (1.6,-6.2) {$G(x)=\sign\Bigl(\sum_{m=1}^M\lambda\alpha_mG_m(x)\Bigr)$};
	%	\draw[-stealth] (GM) to (G);
\end{tikzpicture}
\end{figure}
\end{column}
...
%	\draw [help lines] (-2,-1) grid (2,1); \node[draw,circle] at (0,0) {};
	...
	\draw[-stealth,bend right=35,thick] (-2,0) to (1.7,1.6);
\end{tikzpicture}
